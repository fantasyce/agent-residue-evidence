...
func (s *Store) Sweep(ctx context.Context, now time.Time) error {
	...
	defer s.mu.Unlock()
	now = now.UTC()
	if err := s.sweepOwned(now); err != nil {
		return err
	}
...
func (s *Store) sweepOwned(now time.Time) error {
	for _, directory := range []string{s.tasksDir, s.reportsDir} {
		...
		for _, entry := range entries {
			...
			kept = append(kept, ownedFile{path: path, size: info.Size(), createdAt: envelope.CreatedAt})
			total += info.Size()
		}
