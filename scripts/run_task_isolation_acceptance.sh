...
GOPROXY=off GOSUMDB=off go test -C "$repo_dir" -count=1 \
  -run 'TestPublicIdentifiersNeverAuthorizeCrossTaskOperations|TestReportIDAloneAndWrongOwnerCannotAccessAnotherTask|TestEphemeralObservationStaysInSessionAndCannotResumeAfterRestart' \
  ./internal/mcpserver
GOPROXY=off GOSUMDB=off go test -C "$repo_dir" -count=1 \
